  ...
  Element.prototype.init = function(type){
    var that = this, items = {
      ...
      ,sub_nav: function(){
        ...
        var TIME = 200, timer = {}, timerMore = {}, timeEnd = {}, follow = function(bar, nav, index){
          var othis = $(this), child = othis.find('.layui-sub-nav-child');
            child.addClass(NAV_ANIM);
            child.css({left:child.parents('.layui-sub-nav').width()});
            bar.css({
              left: othis.position().left + parseFloat(othis.css('marginLeft'))
              ,top: othis.position().top + othis.height() - 5
            });
        ...
        $('.layui-sub-nav').each(function(index){
          var othis = $(this)
          ,bar = $('<span class="layui-nav-bar2"></span>')
          ,itemElem = othis.find('.layui-sub-nav-item');
          
          //Hover滑动效果
          if(!othis.find('.layui-nav-bar')[0]){
            othis.append(bar);
            itemElem.on('mouseenter', function(){
              follow.call(this, bar, othis, index);
            }).on('mouseleave', function(){
              if(!othis.hasClass(NAV_TREE)){
                clearTimeout(timerMore[index]);
                timerMore[index] = setTimeout(function(){
                  othis.find('.layui-sub-nav-child').removeClass(SHOW);
                  othis.find('.'+NAV_MORE).removeClass(NAV_MORE+'d');
                }, 300);
              }
            });
          ...
          itemElem.each(function(){
            var oitem = $(this), child = oitem.find('.layui-sub-nav-child');
